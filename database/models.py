...
class HotelExecutive(Base):
    ...
    # Trạng thái kết nối
    # 'Mới tìm thấy', 'Đang xếp hàng', 'Đã gửi kết bạn', 'Đã kết nối', 'Đã bỏ qua'
    status = Column(String(50), default="Mới tìm thấy", index=True)
    lead_score = Column(Integer, default=90)         # GM = 98, DOSM = 95, Marcom = 90
    
    created_at = Column(DateTime, default=datetime.utcnow)
    invited_at = Column(DateTime, nullable=True)     # Thời điểm bấm kết bạn
...
class ConnectionLog(Base):
    ...
    custom_note = Column(Text)                       # Lời nhắn cá nhân hóa đã gửi kèm
...
class SystemSetting(Base):
    """Cấu hình hạn ngạch & mẫu tin nhắn"""
